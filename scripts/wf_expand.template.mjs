...
const Q_ITEM = {
  ...
  required: ['stem', 'options', 'correct_option_id', 'explanation_correct', 'option_rationales', 'source_references', 'support_confidence', 'difficulty'],
  properties: {
...
function genQGroupPrompt(g) {
  const angleLines = g.angles.map((a, i) => `  Question ${i + 1}: focus on ${a}.`).join('\n');
  return `You are an expert Blood Banking / Transfusion Medicine board-question writer. Write EXACTLY ${g.angles.length} distinct one-best-answer MCQ(s) on ONE topic for a physician preparing for the ABPath BB/TM subspecialty exam.

BLUEPRINT TOPIC: ${g.topic_path}
Blueprint node: ${g.blueprint_node_id} · Designation: ${g.designation} (${DESIG[g.designation] || g.designation})

Write one question per angle below — the questions must NOT overlap in what they test:
${angleLines}

APPROVED SOURCES — Read EACH file with the Read tool, fully, before writing. Ground EVERY fact ONLY in them:
${srcList(g)}

STRICT RULES (apply to every question):
1. SOURCE-GROUNDED: every fact in the stem, correct answer, and each distractor rationale must be supported by these approved sources. Do NOT invent facts, numbers, or thresholds absent from the sources. If the sources cannot support a full question for an angle, still return your best grounded attempt and set its support_confidence to "low".
2. FORMAT: focused clinical vignette or scenario stem ending in ONE clear lead-in; 4-5 options; exactly ONE best answer; plausible-but-wrong distractors (prefer the sources' noted board traps).
3. Provide explanation_correct and an option_rationales entry for EVERY option.
4. CITATIONS: source_references cite the actual source_id(s) used with a short paraphrase (<=400 chars, your own words). No long verbatim quotes.
5. Educational board-prep only; not institutional policy. Set support_confidence honestly.

Return {questions:[...]} with exactly ${g.angles.length} item(s), in angle order, via StructuredOutput.`;
}
